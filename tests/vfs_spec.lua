...
describe("lambdamoo.vfs", function()
  ...
  it("leaves remote URI canonicalization to the language server", function()
    local read_uri = nil
    webdav.read_file = function(uri)
      read_uri = uri
      return "player:tell();\n"
    ...
    assert.are.equal("moo://waterpoint/owned/525/verb/check_authorization", vim.api.nvim_buf_get_name(buf))
    assert.are.equal("moo://waterpoint/owned/525/verb/check_authorization", read_uri)
    vim.api.nvim_buf_delete(buf, { force = true })
  end)
end)
